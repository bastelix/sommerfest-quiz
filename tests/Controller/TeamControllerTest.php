...
class TeamControllerTest extends TestCase
{
    public function testPostExceedingTeamLimitReturns402(): void {
        ...
        $pdo->exec(
            'CREATE TABLE tenants(uid TEXT PRIMARY KEY, subdomain TEXT UNIQUE,'
            . ' plan TEXT, custom_limits TEXT, onboarding_state TEXT DEFAULT "pending")'
        );
        $pdo->exec(
            'CREATE TABLE teams(uid TEXT PRIMARY KEY, event_uid TEXT, sort_order INTEGER, name TEXT)'
        );
        $pdo->exec("INSERT INTO events(uid,slug,name) VALUES('e1','e1','Event1')");
        $pdo->exec(
            "INSERT INTO tenants(uid, subdomain, plan, custom_limits, onboarding_state)"
            . " VALUES('t1','sub1','starter', NULL,'completed')"
        );
