...
Alternativ kannst du den Reload durch einen separaten Webhook-Service ausführen lassen. Der Container `nginx-reloader` lauscht auf `http://nginx-reloader:8080/reload` und erwartet das Token `NGINX_RELOAD_TOKEN` im Header `X-Token`. Passe bei Bedarf `NGINX_CONTAINER` an.
