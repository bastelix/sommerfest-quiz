-- Migration 20300106: fix calserver layout – hero padding, stats strip, logo size.
--
-- Problem: Previous edits to namespace-tokens.css on disk were overwritten
-- by DesignTokenService::rebuildStylesheet() which regenerates the file from
-- the config.custom_css column. This migration persists the three fixes:
--   1. Hero section: generous padding instead of compact
--   2. Kennzahlenleiste: less height, larger elements, hidden benefit text
--   3. Logo: larger size (54-82 px instead of 48-72 px)
--
-- The stats-strip selectors use html[data-namespace="calserver"] .calserver-layout
-- to match the marketing.css specificity (body.marketing-page.calserver-layout = 0-3-1).
