(function () {
  ...
  const PROSEAL_TARGET_SELECTOR = selectors.proSealTarget || '[data-proseal-target]';
  const PROSEAL_PLACEHOLDER_SELECTOR = selectors.proSealPlaceholder || '[data-calserver-proseal-placeholder]';
  const PROSEAL_CONSENT_SELECTOR = selectors.proSealConsent || '[data-calserver-proseal-consent]';
  const PROSEAL_ERROR_SELECTOR = selectors.proSealError || '[data-calserver-proseal-error]';
  const VIDEO_SELECTOR = selectors.video || '[data-calserver-video]';
  const VIDEO_CONSENT_SELECTOR = selectors.videoConsent || '[data-calserver-video-consent]';
  ...
  function readPreferences() {
    if (globalThis.marketingCookie && typeof globalThis.marketingCookie.getPreferences === 'function') {
      return globalThis.marketingCookie.getPreferences();
    }
  ...
  function ensureMarketingConsent() {
    ...
    if (globalThis.marketingCookie && typeof globalThis.marketingCookie.allowMarketing === 'function') {
      return globalThis.marketingCookie.allowMarketing();
    }
  ...
  function setupProSealContainer(container) {
    ...
    const target = container.querySelector(PROSEAL_TARGET_SELECTOR);
    if (!target) {
      return;
    }
    ...
    const placeholder = container.querySelector(PROSEAL_PLACEHOLDER_SELECTOR);
    const consentButton = container.querySelector(PROSEAL_CONSENT_SELECTOR);
    const errorMessage = container.querySelector(PROSEAL_ERROR_SELECTOR);
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const containers = document.querySelectorAll(VIDEO_SELECTOR);
    if (!containers.length) {
      return;
    }
    ...
    containers.forEach(function (container) {
      const consentButton = container.querySelector(VIDEO_CONSENT_SELECTOR);
      if (!consentButton) {
        return;
      }
