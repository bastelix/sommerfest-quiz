...
return function (\Slim\App $app, TranslationService $translator) {
    ...
    $app->post('/api/players', function (Request $request, Response $response) {
        ...
        $serverGeneratedUid = false;
        if ($playerUid === '') {
            $playerUid = bin2hex(random_bytes(16));
        ...
        $responsePayload = ['player_uid' => $playerUid];
        $response->getBody()->write((string) json_encode($responsePayload));

        return $response
            ->withHeader('Content-Type', 'application/json')
            ->withStatus($serverGeneratedUid ? 201 : 200);
    });
    ...
    $app->get('/api/quiz-progress', function (Request $request, Response $response) {
        ...
        $resultService = $request->getAttribute('resultService');

        $solved = $resultService->getSolvedCatalogs($eventUid, $playerUid);
        $response->getBody()->write((string) json_encode(['solved' => $solved]));
